/**
 * Email Notification System - 2025 Resend Standards
 * 
 * Follows mind-bank principles:
 * - "Free to Run & Operate" - Resend-optimized delivery
 * - "Secure & Private" - No unnecessary data collection
 * - "Functional Over Flash" - Clear, readable email templates
 * 
 * 2025 Features:
 * - Resend-only implementation (simplified)
 * - Idempotency keys for reliable delivery
...
/**
 * Send email notification about new form submission (2025 Resend-only)
 */
export async function sendNotification(formData, leadId, options = {}) {
  ...
  if (!emailConfig.resend.apiKey) {
    throw new Error('Resend API key is required for 2025 email delivery');
  }
  
  const emailContent = buildEmailContent(formData, leadId);
  
  // Use Resend with 2025 best practices
  return await sendWithResend(emailConfig, emailContent, options);
}

/**
 * Send email using Resend API (2025 Standards)
 */
async function sendWithResend(emailConfig, emailContent, options = {}) {
  try {
    ...
    // Add idempotency key for 2025 reliability
    ...
    console.log('📧 Email sent via Resend with idempotency:', result.id);
    return result;
    
  } catch (error) {
    console.error('❌ Resend email error:', error);
    
    // Retry logic for 2025 reliability
    if (options.retryOnFailure && !options._retryAttempt) {
      console.log('🔄 Retrying email send...');
      await new Promise(resolve => setTimeout(resolve, 1000)); // 1 second delay
      return sendWithResend(emailConfig, emailContent, { ...options, _retryAttempt: 1 });
    }
...
// SMTP functionality removed in 2025 - Resend-only for simplicity
